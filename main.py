...
if __name__ == "__main__":
    ...
    if dataset in h5_datasets:
        ...
        gene_exp, real_label = utils.readCSV(opt.args.name)
        gene_exp = preprocess(gene_exp, opt.args.select_gene)
    ...
    results = train.run(gene_exp=gene_exp, cluster_number=cluster_number,
                        dataset=opt.args.name,
                        real_label=real_label,
                        epochs=opt.args.epoch,
                        lr=opt.args.lr,
                        temperature=opt.args.temperature,
                        dropout=opt.args.dropout,
                        layers=[opt.args.enc_1, opt.args.enc_2, opt.args.enc_3, opt.args.mlp_dim],
                        save_pred=True,
                        cluster_methods=opt.args.cluster_methods,
                        batch_size=opt.args.batch_size,
                        m=opt.args.m,
                        noise=opt.args.noise)

    opt.args.max_epoch = results["max_epoch"]

    print("The best epoch is {}......".format(results["max_epoch"]))
